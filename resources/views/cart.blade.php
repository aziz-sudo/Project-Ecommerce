                ...
                <div class="row g-4">
                    <div class="col-lg-4 col-sm-6">
                        <div class="promo-section">
                            ...
                            <form class="row g-3" method="POST" action="{{route('cart.coupon.apply')}}">
                                @csrf
                                <div class="col-7">
                                ...
                                <div class="col-5">
                                    <button class="btn btn-solid-default rounded btn" type="submit">APPLY COUPON</button>
                                </div>
                            </form>
                            @else
                            <form class="row g-3" method="POST" action="{{route('cart.coupon.remove')}}">
                                @csrf
                                @method('DELETE')
                                <div class="col-7">
                                ...
                                <div class="col-5">
                                    <button class="btn btn-solid-default rounded btn" type="submit">REMOVE COUPON</button>
                                </div>
                            </form>
                            @endif
                    ...
                    <div class="col-lg-4 col-sm-6 ">
                        <div class="checkout-button">
                            <a href="{{route('cart.checkout')}}" class="btn btn-solid-default btn fw-bold">
                            ...
                            <div class="cart-box-details">
                                <div class="total-details">
                                    <div class="top-details">
                                        ...
                                        <h6>Discount{{Session::get('coupon')["code"]}} <span>${{Session::get('discounts')['discount']}}</span></h6>
                                        <h6>Sub Total After Discount <span>${{Session::get('discounts')['subtotal']}}</span></h6>
                                        <h6>Tax <span>${{Session::get('discounts')['tax']}}</span></h6>
                                        <h6>Total <span>${{Session::get('discounts')['total']}}</span></h6>
                                        @else
                                        <h6>Sub Total <span>${{Cart::instance('cart')->subtotal()}}</span></h6>
                                    ...
                                    <div class="bottom-details">
                                        <a href="{{route('cart.checkout')}}">Process Checkout</a>
                                    </div>
                                </div>
                            </div>
                        </div>
